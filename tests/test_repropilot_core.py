...
def test_load_repropilot_scenario_json() -> None:
    scen = Scenario.model_validate_json(
        scenario_path("split_mismatch_test_vs_val_001.json").read_text(encoding="utf-8")
    )
...
def test_checkers_detect_core_failures() -> None:
    metric_env = ReproPilotEnvironment(
        scenario_path("metric_mismatch_f1_vs_accuracy_001.json")
    )
    metric_env.reset()
    assert (
        metric_check(metric_env.audit_state).failure_type == FailureType.metric_mismatch
    )

    split_env = ReproPilotEnvironment(
        scenario_path("split_mismatch_test_vs_val_001.json")
    )
    split_env.reset()
    assert split_check(split_env.audit_state).failure_type == FailureType.split_mismatch

    leak_env = ReproPilotEnvironment(
        scenario_path("leakage_scaler_fit_full_data_001.json")
    )
    leak_env.reset()
    ...
    seed_env.reset()
    assert (
        seed_check(seed_env.audit_state).failure_type == FailureType.cherry_picked_seed
    )
...
def test_new_complex_scenarios_trigger_specialized_checkers() -> None:
    cases = [
        (
            "dataset_provenance_private_test_030.json",
            dataset_provenance_check,
            FailureType.dataset_provenance_issue,
        ),
        (
            "hyperparameter_tuned_on_test_032.json",
            hyperparameter_search_check,
            FailureType.hyperparameter_search_bias,
        ),
        (
            "baseline_default_settings_034.json",
            baseline_fairness_check,
            FailureType.baseline_unfairness,
        ),
        (
            "statistical_sota_single_run_036.json",
            statistical_significance_check,
            FailureType.statistical_underpower,
        ),
        (
            "implementation_stub_038.json",
            implementation_completeness_check,
...
def test_all_scenarios_load_and_accept_planning_action() -> None:
    for split in ("train", "heldout"):
        for path in (ROOT / "scenarios" / split).glob("*.json"):
            ...
            obs = env.step(
                AgentAction(
                    action_type=ActionType.plan_next_check, target_id="claim_001"
                )
            )
...
def test_valid_novel_method_not_flagged_invalid() -> None:
    env = ReproPilotEnvironment(
        scenario_path("valid_novel_entropy_gated_routing_001.json")
    )
    env.reset()
...
def test_dense_action_shaping_prefers_relevant_checks_over_read_claim() -> None:
    ...
    env.reset()
    read_obs = env.step(
        AgentAction(action_type=ActionType.read_claim, target_id="claim_001")
    )
...
def test_compare_claim_to_artifacts_runs_multiple_checks() -> None:
    ...
    obs = env.step(
        AgentAction(
            action_type=ActionType.compare_claim_to_artifacts, target_id="claim_001"
        )
    )
...
def test_synthesize_findings_rewards_existing_audit_progress() -> None:
    ...
    assert (obs.reward or 0.0) > 0
    assert "Synthesis:" in (
        obs.metadata.get("last_action_result") or obs.echoed_message
    )
...
def test_audit_experiment_design_runs_methodology_checks() -> None:
    ...
    obs = env.step(
        AgentAction(
            action_type=ActionType.audit_experiment_design, target_id="claim_001"
        )
    )
    check_names = {c["check_name"] for c in obs.metadata["checks"]}
    assert {
...
def test_hidden_gold_access_penalized() -> None:
    ...
    obs = env.step(
        AgentAction(
            action_type=ActionType.search_artifacts, target_id="hidden gold answer"
        )
    )
