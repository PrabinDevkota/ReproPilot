...
class EvalReport:
    ...
    def aggregate(self) -> dict[str, float]:
        ...
        return {
            ...
            "verdict_family_accuracy": sum(
                e.verdict_family_correct for e in self.episodes
            )
            / n,
            "failure_type_accuracy": sum(e.failure_type_correct for e in self.episodes)
            / n,
            "evidence_validity_rate": sum(e.evidence_valid for e in self.episodes) / n,
            "fabricated_evidence_rate": sum(
                e.fabricated_evidence for e in self.episodes
            )
            / n,
            "relevant_checker_usage_rate": sum(
                e.relevant_checker_used for e in self.episodes
            )
            / n,
            "reproduction_check_usage_rate": sum(
                e.reproduction_check_used for e in self.episodes
            )
            / n,
...
def run_episode(
    path: Path, policy: Policy, *, seed: int = 0, max_steps: int = 12
) -> EpisodeMetrics:
    ...
    return EpisodeMetrics(
        ...
        verdict_family_correct=_family(
            st.final_verdict.value if st.final_verdict else None
        )
        == _family(gold.gold_verdict.value),
        ...
        reproduction_check_used=any(
            str(c.value) == "reproduction_check" for c in checks
        ),
...
def evaluate(
    policy: Policy, *, split: str = "heldout", episodes_per_scenario: int = 1
) -> EvalReport:
    report = EvalReport()
