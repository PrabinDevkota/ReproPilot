...
try:
    from ..models import ActionType, AgentAction, CheckName, FailureType, ReproPilotObservation, ValidationVerdict
except ImportError:
    from models import ActionType, AgentAction, CheckName, FailureType, ReproPilotObservation, ValidationVerdict
...
def _history(meta: dict[str, Any]) -> set[str]:
    return {str(c.get("check_name")) for c in meta.get("checks") or [] if isinstance(c, dict)}
...
def _failed_checks(meta: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        ...
        if isinstance(c, dict) and c.get("status") in {"failed", "inconclusive"} and c.get("failure_type")
    ]


def _observed_evidence(meta: dict[str, Any]) -> list[str]:
    return [str(e.get("id")) for e in meta.get("observed_evidence") or [] if isinstance(e, dict)]


def smart_action(obs: ReproPilotObservation, rng: random.Random | None = None) -> AgentAction:
    meta: dict[str, Any] = obs.metadata or {}
    text = obs.echoed_message.lower()
    checks_done = _history(meta)
    step = int(meta.get("episode_step") or 0)

    if step == 0:
        return AgentAction(action_type=ActionType.read_claim, explanation="Start by reading the target claim.")
    ...
    for fid in meta.get("code_file_ids") or []:
        if fid not in inspected_files:
            return AgentAction(action_type=ActionType.inspect_code_file, target_id=fid, explanation="Inspect code before running methodology checks.")

    for cid in meta.get("config_ids") or []:
        if cid not in inspected_cfgs:
            return AgentAction(action_type=ActionType.inspect_config, target_id=cid, explanation="Inspect config for split and metric.")

    for lid in meta.get("log_ids") or []:
        if lid not in inspected_logs:
            return AgentAction(action_type=ActionType.inspect_logs, target_id=lid, explanation="Inspect logs for metric values and seed behavior.")

    failed = _failed_checks(meta)
    if failed:
        evidence_ids = _observed_evidence(meta)
        failure = FailureType(str(failed[0].get("failure_type")))
        verdict = ValidationVerdict.NOT_SUPPORTED_RESULT_MISMATCH if failure == FailureType.result_mismatch else ValidationVerdict.NOT_SUPPORTED_METHOD_INVALID
        if failure == FailureType.missing_artifact:
            verdict = ValidationVerdict.INCONCLUSIVE_MISSING_ARTIFACTS
        return AgentAction(action_type=ActionType.submit_verdict, verdict=verdict, failure_type=failure, evidence_ids=evidence_ids, explanation="A deterministic checker found a methodology issue.")
    ...
    if int(meta.get("steps_remaining") or 0) <= 2 and checks_done:
        ...
        return AgentAction(action_type=ActionType.submit_verdict, verdict=verdict, failure_type=FailureType.none, evidence_ids=evidence_ids, explanation="No checker has failed within the available audit budget.")
    ...
    for name in priority:
        if name not in checks_done:
            return AgentAction(action_type=_CHECK_TO_ACTION[name], target_id=meta.get("claim_id"), explanation=f"Run {name} before verdict.")
    ...
    return AgentAction(action_type=ActionType.submit_verdict, verdict=verdict, failure_type=FailureType.none, evidence_ids=evidence_ids, explanation="Available checks passed.")
