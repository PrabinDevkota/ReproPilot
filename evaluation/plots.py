    ...
    *,
    ...
    try:
        ...
        ax.bar(
            labels,
            values,
            color=["#2563eb", "#16a34a", "#dc2626", "#9333ea"][: len(labels)],
        )
        ax.set_title(title)
        if ylim:
            ax.set_ylim(*ylim)
...
def generate_plots(report_path: Path, out_dir: Path) -> list[Path]:
    ...
    paths = [
        _bar(
            out_dir / "total_reward_before_after.png",
            "Average Total Reward",
            ["policy"],
            [float(agg.get("average_total_reward", 0.0))],
        ),
        _bar(
            out_dir / "verdict_accuracy_before_after.png",
            "Verdict Accuracy",
            ["policy"],
            [float(agg.get("verdict_accuracy", 0.0))],
            ylim=(0, 1),
        ),
        _bar(
            out_dir / "failure_type_accuracy.png",
            "Failure Type Accuracy",
            ["policy"],
            [float(agg.get("failure_type_accuracy", 0.0))],
            ylim=(0, 1),
        ),
        _bar(
            out_dir / "evidence_validity_rate.png",
            "Evidence Validity Rate",
            ["policy"],
            [float(agg.get("evidence_validity_rate", 0.0))],
            ylim=(0, 1),
        ),
        _bar(
            out_dir / "fabricated_evidence_rate.png",
            "Fabricated Evidence Rate",
            ["policy"],
            [float(agg.get("fabricated_evidence_rate", 0.0))],
            ylim=(0, 1),
        ),
        _bar(
            out_dir / "checker_usage_rate.png",
            "Relevant Checker Usage",
            ["policy"],
            [float(agg.get("relevant_checker_usage_rate", 0.0))],
            ylim=(0, 1),
        ),
    ]
    ...
    novelty_ok = sum(
        1
        for e in episodes
        if "novel" in str(e.get("scenario_id", "")) and e.get("verdict_correct")
    )
    paths.append(
        _bar(
            out_dir / "novelty_calibration.png",
            "Novelty Calibration",
            ["novel"],
            [novelty_ok / max(1, novelty_total)],
            ylim=(0, 1),
        )
    )
    ...
    for e in episodes:
        ...
        key = (
            sid.split("_")[1]
            if sid.startswith("heldout_") and "_" in sid
            else sid.split("_")[0]
        )
        by_type.setdefault(key, []).append(
            1.0 if e.get("verdict_family_correct") else 0.0
        )
    labels = sorted(by_type)[:10]
    vals = [sum(by_type[k]) / max(1, len(by_type[k])) for k in labels]
    paths.append(
        _bar(
            out_dir / "scenario_type_breakdown.png",
